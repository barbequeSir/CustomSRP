...
CBUFFER_START(_CustomSHADOW)
    ...
    int _CascadeCount;
    float4 _CascadeCullingSheres[MAX_CASCADES];
...
float GetDirectionalShadowAttenuation(DirectionalShadowData directionalShadowData,ShadowData global,Surface surface)
{
    ...
    {
        normalBias = surface.normal * (directionalShadowData.normalBias + _CascadeData[global.cascadeIndex + 1].y);
        positionSTS = mul(_DirectionalShadowMatrices[directionalShadowData.tileIndex + 1],float4(surface.position + normalBias,1.0f)).xyz;
...
ShadowData GetShadowData(Surface surface)
{
    ...
    {
        float4 sphere = _CascadeCullingSheres[i];
